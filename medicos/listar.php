...
$stmt = $conexao->query("SELECT * FROM medicos ORDER BY nome ASC");
...
<div class="max-w-7xl mx-auto px-4 py-8">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-gray-500 text-sm mt-1">Lista de todos os médicos cadastrados</p>
        </div>
        <a href="cadastrar.php" class="bg-blue-800 text-white text-sm font-semibold px-4 py-2 rounded-lg hover:bg-blue-700 transition">+ Novo Médico</a>
    </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b">
                <tr>
                    ...
                    <th class="text-left px-5 py-3 text-gray-500 font-medium">CRM</th>
            ...
            <tbody class="divide-y divide-gray-100">
                <?php if (empty($medicos)): ?>
                    <tr>
                        <td colspan="6" class="text-center text-gray-400 py-8">Nenhum médico cadastrado ainda.</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($medicos as $m): ?>
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-5 py-3 text-gray-400"><?= $m['id'] ?></td>
                        <td class="px-5 py-3 font-medium text-gray-800"><?= htmlspecialchars($m['nome']) ?></td>
                        <td class="px-5 py-3 text-gray-600"><?= htmlspecialchars($m['crm']) ?></td>
                        <td class="px-5 py-3 text-gray-600"><?= htmlspecialchars($m['especialidade']) ?></td>
                        <td class="px-5 py-3 text-gray-600"><?= htmlspecialchars($m['telefone']) ?></td>
                        <td class="px-5 py-3 flex gap-2">
                            <a href="editar.php?id=<?= $m['id'] ?>" class="bg-yellow-100 text-yellow-700 text-xs px-3 py-1 rounded-lg hover:bg-yellow-200 transition">Editar</a>
                            <a href="excluir.php?id=<?= $m['id'] ?>" onclick="return confirm('Tem certeza que deseja excluir este médico?')" class="bg-red-100 text-red-600 text-xs px-3 py-1 rounded-lg hover:bg-red-200 transition">Excluir</a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
