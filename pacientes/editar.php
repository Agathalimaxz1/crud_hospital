...
$stmt = $conexao->prepare("SELECT * FROM pacientes WHERE id = :id");
$stmt->execute([':id' => $id]);
$paciente = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nome) || empty($cpf) || empty($data_nascimento) || empty($telefone) || empty($endereco)) {
        ...
        $stmt = $conexao->prepare("SELECT id FROM pacientes WHERE cpf = :cpf AND id != :id");
        $stmt->execute([':cpf' => $cpf, ':id' => $id]);
        if ($stmt->fetch()) {
            $erro = "Já existe outro paciente com esse CPF.";
        } else {
            $stmt = $conexao->prepare("UPDATE pacientes SET nome = :nome, cpf = :cpf, data_nasc = :data_nasc, telefone = :telefone, endereco = :endereco WHERE id = :id");
            $stmt->execute([
                ':nome'            => $nome,
                ':cpf'             => $cpf,
            ...
            header('Location: listar.php?sucesso=Paciente atualizado com sucesso!');
            exit;
        }
    }
}
...
<head>
    ...
    <title>Editar Paciente — HSJ</title>
...
<div class="max-w-2xl mx-auto px-4 py-8">
    <div class="mb-6">
        ...
        <p class="text-gray-500 text-sm mt-1">Altere os dados do paciente</p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow p-6">
        <form method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="nome" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition" value="<?= htmlspecialchars($paciente['nome']) ?>">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">CPF</label>
                <input type="text" name="cpf" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition" value="<?= htmlspecialchars($paciente['cpf']) ?>">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">Data de nascimento</label>
                <input type="date" name="data_nasc" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition" value="<?= htmlspecialchars($paciente['data_nasc']) ?>">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">Telefone</label>
                <input type="text" name="telefone" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition" value="<?= htmlspecialchars($paciente['telefone']) ?>">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">Endereço</label>
                <input type="text" name="endereco" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition" value="<?= htmlspecialchars($paciente['endereco']) ?>">
            </div>
